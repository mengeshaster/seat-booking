...
async function applyMigrations(): Promise<void> {
    ...
    try {
        ...
        const applied = await client.query<{ name: string }>(
            "SELECT name FROM notification_schema_migrations"
        );
        ...
        for (const migrationName of migrationNames) {
            ...
            try {
                ...
                await client.query(
                    "INSERT INTO notification_schema_migrations (name) VALUES ($1)",
                    [migrationName]
                );
                await client.query("COMMIT");
                console.log(`Applied notification migration ${migrationName}`);
            } catch (error) {
